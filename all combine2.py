...
# -------------------------------------------------------------
# LABEL DETECTIONS LEFT TO RIGHT
# -------------------------------------------------------------
def label_detections_left_to_right(detections):
    """Sort detections by x-coordinate (left to right) and add labels."""
...
def detect(model, img):
    ...
    for box in r.boxes:
        ...
        dets.append({"cx": cx, "cy": cy, "conf": conf, "class_name": name, "x1": x1, "y1": y1, "x2": x2, "y2": y2})

    # Sort detections left to right and add labels
    if dets:
        dets = label_detections_left_to_right(dets)
        
        # Draw after sorting and labeling
        for det in dets:
            # Draw bounding box
            cv2.rectangle(img, (det['x1'], det['y1']), (det['x2'], det['y2']), (0, 0, 255), 2)
            
            # Draw center point
            cv2.circle(img, (det['cx'], det['cy']), 6, (0, 255, 0), -1)
            
            # Draw label and confidence
            label_text = f"{det['label']}. {det['class_name']} {det['conf']:.2f}"
